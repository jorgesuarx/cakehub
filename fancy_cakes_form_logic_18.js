...
function setDateTime() {
  const now = new Date();
  const twoDaysFromNow = addDays(now, 4); 
  ...
  datetimeInput.addEventListener('change', function() {
    ...
    if (!timeIsValid) {
      timeErrorMessage = showErrorMessage(timeErrorMessage, 'Outside working hours. Please select a time within 9AM-5PM.');
    } else {
    ...
    if (!dateIsValid) {
      dateErrorMessage = showErrorMessage(dateErrorMessage, `Unavailable. The next open date  is ${formatDate(twoDaysFromNow)}. For rush orders, call us at (863) 268-8441.`);
