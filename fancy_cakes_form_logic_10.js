...
document.addEventListener("DOMContentLoaded", function() {
  ...
  var sections = ["cakepops", "cupcakes", "cakesicles", "cake-jars", "pastries", "flan", "flan", "cookies", "tres-leches-shoots", "traditional-tres-leches", "chocolate-dipped-treats", "custom-cake"];
...
function setDateTime() {
  ...
  datetimeInput.addEventListener('change', function() {
    ...
    if (!timeIsValid) {
      timeErrorMessage = showErrorMessage(timeErrorMessage, 'Invalid time. Please select a time within our working hours (10AM-5PM).');
    } else {
    ...
    if (!dateIsValid) {
      dateErrorMessage = showErrorMessage(dateErrorMessage, `Invalid date. Please select a date after ${formatDate(twoDaysFromNow)}.`);
